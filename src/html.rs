...
pub fn render_html(rep: &ReportSummary, theme: crate::Theme, use_emoji: bool, tz: TimeZone, tfmt: Option<&str>, lang: Lang) -> String {
    ...
    for e in &rep.samples {
        ...
        let dev = crate::device_map::friendly_device(&dev_raw).unwrap_or(dev_raw.clone());
        let dev_cls = crate::device_map::classify_instance_id(&dev_raw).unwrap_or_default();
        let dev_disp = if dev_cls.is_empty() { html_escape(&dev) } else { format!("{} <span class=\"pill\">{}</span>", html_escape(&dev), html_escape(&dev_cls)) };
