...
pub fn render_html(rep: &ReportSummary, theme: crate::Theme, use_emoji: bool, tz: TimeZone, tfmt: Option<&str>) -> String {
    ...
    s.push_str("<div class=\"section\"><h3>Recent Samples</h3><table class=\"table\"><thead><tr><th>Time</th><th>Channel</th><th>Provider</th><th>Device</th><th>Event ID</th><th>Cause</th><th>Message</th></tr></thead><tbody>");
    for e in &rep.samples {
        ...
        if msg.chars().count() > 240 {
            s.push_str(&format!("<tr><td class=\"sub\">{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td><span class=\"ellipsis\">{}</span><details><summary>Show full</summary><div class=\"code\">{}</div></details></td></tr>", ts, html_escape(&e.channel), html_escape(&e.provider), html_escape(&dev), e.event_id, html_escape(&cause_from(e)), html_escape(&truncated), html_escape(msg)));
        } else {
            s.push_str(&format!("<tr><td class=\"sub\">{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>", ts, html_escape(&e.channel), html_escape(&e.provider), html_escape(&dev), e.event_id, html_escape(&cause_from(e)), html_escape(msg)));
        }
    }
    s.push_str("</tbody></table></div>");
